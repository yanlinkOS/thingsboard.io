...
description: Build a complete ThingsBoard IoT Hub solution template — device profiles, dashboards, rule chains, and emulated telemetry — that end users install in one click.
---
...
- [ ] `devices.json`, `assets.json`, `customers.json`, `dashboards.json`, `rule_chains.json`, `device_emulators.json` all exist (maybe `[]`)
...
- [ ] Explains who the solution template is for and the scenario it covers
- [ ] Lists every entity included in the bundle (devices, dashboards, rule chains, calculated fields)
- [ ] Documents how to set up the solution and customize it after installation

### Content Quality

- [ ] Solution template installs cleanly on a fresh ThingsBoard tenant
- [ ] Preview image shows the main dashboard with realistic data, not placeholders
...
Reviewers verify that the submission meets the same criteria as the [Pre-Upload Checklist](#pre-upload-checklist): the solution template installs cleanly on a fresh tenant, entity declarations and dashboard exports are correct, post-install instructions are complete and accurate, and the listing and readme give users enough context to evaluate and use the template.
