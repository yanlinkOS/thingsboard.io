...
<Aside type="note">
  On the nodes you add, set `debugSettings` with `failuresEnabled` as `true` and `allEnabled` as `false` — failed messages then produce debug events without the overhead of tracing every message. Leave the default-processing nodes' debug settings as they are.
</Aside>
...
- [ ] Explains who the rule chain is for and the integration scenario it covers
- [ ] Describes the message flow and the role of each significant node
- [ ] Lists external requirements (broker, credentials, target system)
- [ ] Documents how to install, configure, and link the chain to device profiles

### Content Quality

- [ ] Rule chain has been tested end-to-end with a real test message
...
Reviewers verify that the submission meets the same criteria as the [Pre-Upload Checklist](#pre-upload-checklist): the rule chain processes messages correctly, the JSON is valid, and the listing and readme give users enough context to evaluate and use the chain.
...
- <a href="/docs/iot-hub/user-guides/rule-chains/" target="_blank" rel="noopener noreferrer"><strong>Rule Chains</strong></a> — how end users discover and install a rule chain
