---
title: Rule chain contribution guide
description: Contribute a rule chain to the ThingsBoard IoT Hub so end users install ready-made data-processing logic on their instance in a single click.
---
...
2. Export it as a JSON file — see [Export the rule chain](#export-the-rule-chain).
...
Each rule chain version is identified by a semver string (e.g. `1.0.0`) plus a changelog entry, and a SHA-256 **checksum** computed from:
