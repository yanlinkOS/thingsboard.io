...
| `${integration.httpEndpoint}` | `INTEGRATION` | The webhook URL the integration listens on (PE only). Reference it in `post-install.md` to display the URL the customer registers with their LoRaWAN provider, MQTT broker, etc. |
