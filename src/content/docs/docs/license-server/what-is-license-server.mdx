...
description: Features and advantages of ThingsBoard License Server — hardware-agnostic licensing, pay-as-you-go subscriptions, perpetual fallback licenses, and Stripe-secured payments.
---
...
If you are interested to use it for selling your software, please [contact us](/contact-us/).
...
ThingsBoard also provides the ability to download digital copies of the invoices.
...
These requests validate that the subscription / perpetual license is valid and the number of launched instances does not exceed the subscription threshold.
...
During the first launch of ThingsBoard PE, the built-in License Server Client generates an "Activate Instance Request" to the License Server.
This request contains the license key and version info about the current platform installation.
License Server looks up the subscription info based on the license key and replies with the instance ID, subscription plan data, and some magic bytes.
License Client stores this information locally and uses the instance ID and some magic bytes for the next license check requests.

<ImageGallery images={[{ src: '/src/assets/images/license-server/license-activation.gif', alt: 'License activation flow' }]} />
...
In case of a successful request, the client may receive an update to the subscription plan data. This may be caused by an update of the subscription plan.

<ImageGallery images={[{ src: '/src/assets/images/license-server/license-check.gif', alt: 'License check flow' }]} />