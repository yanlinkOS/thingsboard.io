...
Use the address to access the HTTP web UI (port 80) and connect devices via <DocLink product={Products.PE} path="reference/http-api/getting-connected">HTTP API</DocLink>.
...
Follow the <DocLink product={Products.PE} path="reference/mqtt-api/getting-connected#mqtt-over-tls">MQTT over SSL</DocLink> guide to create a `.pem` file.
