...
Use the address to access the web UI (port 80) and connect devices via <DocLink product={Products.CE} path="reference/http-api">HTTP API</DocLink>.
...
Follow the <DocLink product={Products.CE} path="user-guide/ssl/http-over-ssl">HTTP over SSL</DocLink> guide to configure SSL in `tb-node.yml`. Then deploy:
...
For MQTT over SSL, follow the <DocLink product={Products.CE} path="user-guide/mqtt-over-ssl">MQTT over SSL</DocLink> guide to configure the required environment variables in `tb-node.yml`.
...
For CoAP over DTLS, follow the <DocLink product={Products.CE} path="user-guide/coap-over-dtls">CoAP over DTLS</DocLink> guide. For LwM2M over DTLS, follow the <DocLink product={Products.CE} path="user-guide/ssl/lwm2m-over-dtls">LwM2M over DTLS</DocLink> guide.
