...
The **Export Dashboard** feature relies on **tb-web-report** — a service that runs a headless Chromium browser to render dashboards and capture them as PDF or PNG.
Most issues with the Dashboard export process are caused by timeout values that are too low for the complexity of the dashboard being rendered.

<Aside type="caution">
  **tb-web-report is a legacy component.** For better performance and reliability, we recommend migrating to the new Reporting feature with Report Templates, introduced in ThingsBoard 4.2. See the <DocLink product={Products.PE} path="user-guide/reporting/getting-started/">Reporting documentation</DocLink> for details.
</Aside>
...
Logs go to stdout. No log file is written when `DOCKER_MODE=true`. To view recent log entries:
...
Add or update in the `environment` section of your `docker-compose.yml`:
...
Available levels from least to most verbose: `error` → `warn` → `info` → `http` → `verbose` → `debug` → `silly`

Remember to set the level back to `info` after diagnosing, then restart tb-web-report.

For the full reference of Report Service configuration parameters, see the <DocLink product={Products.PE} path="reference/configuration/report-service-config">Report Service configuration reference</DocLink>.
...
Add or update in the `environment` section of your ThingsBoard container in `docker-compose.yml`:
...
<Aside type="caution">
  `DASHBOARD_LOAD_WAIT_TIME` is **not** a valid variable name. The correct name is `DASHBOARD_IDLE_WAIT_TIME`.
</Aside>
