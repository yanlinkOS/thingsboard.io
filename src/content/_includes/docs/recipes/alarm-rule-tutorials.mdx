...
These tutorials walk through common alarm rule configurations using a Thermometer device that sends `temperature` telemetry. Each example builds on the previous one. For the full configuration reference, see <DocLink product={props.product === Products.EDGE_PE ? Products.PE : props.product === Products.EDGE ? Products.CE : props.product} path="user-guide/alarm-rules" target="_blank">Alarm rules</DocLink>.
...
Download the setup file and <DocLink product={props.product === Products.EDGE_PE ? Products.PE : props.product === Products.EDGE ? Products.CE : props.product} path="user-guide/provisioning#bulk-provisioning" target="_blank">import</DocLink> it before running the examples:
...
**Preparation:** Add a <DocLink product={props.product === Products.EDGE_PE ? Products.PE : props.product === Products.EDGE ? Products.CE : props.product} path="user-guide/digital-twins/attributes/#server-side-attributes" target="_blank">server attribute</DocLink> to *Thermometer*:
