...
import symbolImg from '~/assets/images/reference/widgets/scada/general-high-performance-scada/symbol-hp-consumers.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Consumers SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '159px', objectFit: 'contain' }}
/>

The **HP Consumers** widget represents a group of end consumers — residential, commercial, or industrial buildings that draw electricity, gas, water, or heat from a distribution network. The widget shows whether the consumer group is actively being supplied (running state) and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on smart grid, district heating, water distribution, or utility management dashboards to visualize load zones and surface supply or demand alerts directly on the diagram.

</div>

## Key capabilities

- Indicate active supply to the consumer group in the running state.
- Indicate warning and critical states with configurable colors.
- Animate a blinking effect when the consumer group is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Consumers widget has a pre-selected symbol from the **General high-performance SCADA symbols** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the consumer group is currently being actively supplied. When `True`, the running state is shown. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the consumer group is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the consumer group is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the consumer group is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getSupplyStatus`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `consumersActive` or `supplyState`. |
...
The Appearance section controls the widget title, consumer group state colors, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Consumers` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Running color** | Light gray | Consumer group color when actively supplied (running state). |
| **Stopped color** | Dark gray | Consumer group color when not being supplied. |
| **Warning color** | Orange | Consumer group color when in warning state. |
| **Critical color** | Red | Consumer group color when in critical state. |
...
<p style="font-size: 1.1em; font-weight: bold;">Consumer group shows no running state</p>
