...
import symbolImg from '~/assets/images/reference/widgets/scada/general-high-performance-scada/symbol-hp-drawwork.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Drawwork SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '159px', objectFit: 'contain' }}
/>

The **HP Drawwork** widget represents an industrial drawwork — the hoisting drum assembly used in drilling rigs to raise and lower the drill string. The widget supports drum rotation animation when running, per-state click actions for warning and critical alarm conditions, and a unified blinking animation covering both alarm states. 
  
Use it on oil and gas drilling dashboards to monitor and interact with the hoisting system in real time.

</div>

## Key capabilities

- Animate the drawwork drum rotation when the unit is in the running state.
- Indicate warning and critical states with configurable colors.
- Animate a blinking effect when the drawwork is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Drawwork widget has a pre-selected symbol from the **General high-performance SCADA symbols** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the drawwork is currently running. When `True`, the drum rotation animation plays. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the drawwork is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the drawwork is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the drawwork is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getDrawworkStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `drawworkState`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `drawworkRunning` or `hoistStatus`. |
...
The Appearance section controls the widget title, drawwork state colors, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Drawwork` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Running color** | Light gray | Drawwork color when in the running state. |
| **Stopped color** | Dark gray | Drawwork color when stopped (not running). |
| **Warning color** | Orange | Drawwork color when in warning state. |
| **Critical color** | Red | Drawwork color when in critical state. |
...
<p style="font-size: 1.1em; font-weight: bold;">Drawwork shows no rotation animation when running</p>
...
| Cause | Solution |
|---|---|
| The drawwork is not currently in the expected alarm state | The **Warning click** handler only fires when the **Warning** state is active; **Critical click** only fires when the **Critical** state is active. Verify the alarm is active on the device. |
