...
import symbolImg from '~/assets/images/reference/widgets/scada/general-high-performance-scada/symbol-hp-crane.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Crane SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '199px', objectFit: 'contain' }}
/>

The **HP Crane** widget represents an industrial crane rendered as a tower-and-boom assembly. The widget supports motion animation when the crane is operating, per-state click actions for warning and critical alarm conditions, and a unified blinking animation covering both alarm states. 
  
Use it on construction site, port, warehouse, or heavy industry dashboards to monitor crane status and respond to alarms in real time.

</div>

## Key capabilities

- Animate the crane when the unit is in the running state.
- Indicate warning and critical states with configurable colors.
- Animate a blinking effect when the crane is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Crane widget has a pre-selected symbol from the **General high-performance SCADA symbols** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the crane is currently operating. When `True`, the running animation plays. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the crane is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the crane is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the crane is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getCraneStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `craneState`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `craneRunning` or `operatingStatus`. |
...
The Appearance section controls the widget title, crane state colors, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Crane` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Running color** | Light gray | Crane color when in the running state. |
| **Stopped color** | Dark gray | Crane color when stopped (not running). |
| **Warning color** | Orange | Crane color when in warning state. |
| **Critical color** | Red | Crane color when in critical state. |
...
<p style="font-size: 1.1em; font-weight: bold;">Crane shows no animation when running</p>
...
| Cause | Solution |
|---|---|
| The crane is not currently in the expected alarm state | The **Warning click** handler only fires when the **Warning** state is active; **Critical click** only fires when the **Critical** state is active. Verify the alarm is active on the device. |
