...
import symbolImg from '~/assets/images/reference/widgets/scada/general-high-performance-scada/symbol-hp-hook.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Hook SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '199px', objectFit: 'contain' }}
/>

The **HP Hook** widget represents an industrial lifting hook — the load-bearing end fitting of a crane, hoist, or overhead lifting system. Unlike motorized equipment, a hook has no running state of its own; the widget focuses on alarm monitoring, showing warning and critical conditions with configurable colors, a blinking animation, and per-state click actions. Use it on crane, hoist, or material-handling dashboards to surface overload alarms, load limit violations, or safety interlocks directly on the diagram.
...
- Animate a blinking effect when the hook is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Hook widget has a pre-selected symbol from the **General high-performance SCADA symbols** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Warning** | `Use alarm status` | Whether the hook is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the hook is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the hook is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
Applies to **Warning**, **Critical**, and **Warning/Critical state animation**. Click the edit icon next to a parameter to open its configuration dialog. When **Action** is set to **Do nothing**, select `True` or `False` as the fixed value.
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getHookStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `overloaded` or `hookWarning`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `loadWarning` or `hookAlarm`. |
...
The Appearance section controls the widget title, hook color, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Hook` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Hook color** | White | Base color of the hook symbol. |
| **Warning color** | Orange | Hook color when in warning state. |
| **Critical color** | Red | Hook color when in critical state. |
...
| Cause | Solution |
|---|---|
| The hook is not currently in the expected alarm state | The **Warning click** handler only fires when the **Warning** state is active; **Critical click** only fires when the **Critical** state is active. Verify the alarm is active on the device. |
