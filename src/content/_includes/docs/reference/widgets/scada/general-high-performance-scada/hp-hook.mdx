...
It represents an industrial lifting hook — the load-bearing end fitting of a crane, hoist, or overhead lifting system. Unlike motorized equipment, a hook has no running state of its own; the widget focuses on alarm monitoring, showing warning and critical conditions with configurable colors, a blinking animation, and per-state click actions. 

Use it on crane, hoist, or material-handling dashboards to surface overload alarms, load limit violations, or safety interlocks directly on the diagram.
