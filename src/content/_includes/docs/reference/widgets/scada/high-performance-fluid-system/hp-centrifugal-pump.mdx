...
import symbolImg from '~/assets/images/reference/widgets/scada/high-performance-fluid-system/symbol-hp-centrifugal-pump.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Centrifugal pump SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '199px', objectFit: 'contain' }}
/>

The **HP Centrifugal pump** widget represents an industrial centrifugal pump rendered as a front-facing assembly with a visible motor housing, volute casing, and impeller. The widget supports impeller rotation animation, per-state click actions for both warning and critical conditions, and a unified blinking animation covering both alarm states.
...
The SCADA symbol defines the visual shape of the widget. The HP Centrifugal pump widget has a pre-selected symbol from the **High-performance SCADA fluid system** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the pump is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
|---|---|---|
| **Title** | `HP Centrifugal pump` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
...
| Cause | Solution |
|---|---|
| The pump is not currently in the expected alarm state | The **Warning click** handler only fires when the **Warning** state is active; **Critical click** only fires when the **Critical** state is active. Verify the alarm is active on the device. |
