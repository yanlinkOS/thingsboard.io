...
import symbolImg from '~/assets/images/reference/widgets/scada/high-performance-fluid-system/symbol-hp-sand-filter.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Sand filter SCADA symbol"
  width={150}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '150px', height: '250px', objectFit: 'contain' }}
/>

The **HP Sand filter** widget represents an industrial multi-port sand filter with a built-in mode selector panel. The widget displays the current operational mode — Filter, Backwash, Rinse, Waste, Recirculate, or Closed — and lets operators switch between modes by clicking a mode button directly on the widget face. Use it in swimming pool systems, water treatment plants, or any industrial process where sand filter mode control and real-time status visibility are needed from a single dashboard widget.

</div>

## Key capabilities

- Display the current filtration mode with the active mode button highlighted.
...
The SCADA symbol defines the visual shape of the widget. The HP Sand filter widget has a pre-selected symbol from the **High-performance SCADA fluid system** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the sand filter is currently running. See [Data source configuration](#data-source-configuration). |
| **Filtration mode** | `Use attribute 'filtrationMode'` | The current operational mode of the filter. The matching mode button is highlighted on the widget face. See [Data source configuration](#data-source-configuration). |
| **Filtration mode update state** | `Set 'filtrationMode' attribute to: value` | Action triggered when the operator clicks a mode button on the widget face to switch the filtration mode. See [Update action configuration](#update-action-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the filter is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the filter is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the filter is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
Applies to **Filtration mode update state**. This action is triggered each time the operator clicks one of the mode buttons on the widget face to switch the filtration mode. The value written is the identifier of the selected mode. Select one action type from the list below.
...
- **Execute RPC**
  ...
  | **Parameters** | **Value** — sends the mode identifier directly; **Function** — sends a transformed value computed by your `f(value)` function; **None** — sends no parameter. |
...
- **Set attribute**
  ...
  | **Attribute key** | Attribute name to write to, like `filtrationMode`. |
  | **Value** | **Value** — writes the mode identifier directly; **Function** — writes a transformed value computed by your `f(value)` function. |
...
- **Add time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | The telemetry key to write, like `filtrationMode`. |
  | **Value** | **Value** — writes the mode identifier directly; **Function** — writes a transformed value computed by your `f(value)` function. |

### Data source configuration

Applies to **Running**, **Filtration mode**, **Warning**, **Critical**, and **Warning/Critical state animation**. Click the edit icon next to a parameter to open its configuration dialog. When **Action** is set to **Do nothing**, the **Value** options depend on which parameter you are configuring:
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `filtrationMode` or `filterRunning`. |
...
The Appearance section controls the widget title, which mode buttons are shown, their colors, and the visual style of the active mode indicator.
...
<p style="font-size: 1.1em; font-weight: bold;">No mode button is highlighted as active</p>
...
<p style="font-size: 1.1em; font-weight: bold;">Clicking a mode button does not update the filter mode</p>
