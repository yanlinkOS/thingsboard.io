...
It represents an industrial filter canister rendered as a front-facing vertical vessel. Unlike pump or heat pump widgets, the filter has no running state — it serves as a visual alarm indicator in your system diagram, changing color and optionally blinking when warning or critical conditions are active. Use it to mark filtration stages in heating, cooling, water treatment, or process fluid systems where filter condition monitoring matters.
