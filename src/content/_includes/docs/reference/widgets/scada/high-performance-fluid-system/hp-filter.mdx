...
import symbolImg from '~/assets/images/reference/widgets/scada/high-performance-fluid-system/symbol-hp-filter.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Filter SCADA symbol"
  width={100}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '100px', height: '200px', objectFit: 'contain' }}
/>

The **HP Filter** widget represents an industrial filter canister rendered as a front-facing vertical vessel. Unlike pump or heat pump widgets, the filter has no running state — it serves as a visual alarm indicator in your system diagram, changing color and optionally blinking when warning or critical conditions are active. Use it to mark filtration stages in heating, cooling, water treatment, or process fluid systems where filter condition monitoring matters.
...
- Set an optional filter body color applied when no alarm is active.
...
The SCADA symbol defines the visual shape of the widget. The HP Filter widget has a pre-selected symbol from the **High-performance SCADA fluid system** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.

## Behavior

Behavior parameters define how the widget responds to user interaction, driven by real-time data from the connected device.
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getFilterStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `filterBlocked` or `needsReplacement`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `filterPressureDrop` or `filterState`. |
...
The Appearance section controls the widget title and filter colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Filter` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Filter color** | Off | Optional fill color for the filter body. When enabled, this color is applied in the normal (non-alarm) state. |
