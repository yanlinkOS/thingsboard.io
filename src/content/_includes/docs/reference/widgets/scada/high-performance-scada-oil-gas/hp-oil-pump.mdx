...
import symbolImg from '~/assets/images/reference/widgets/scada/high-performance-scada-oil-gas/symbol-hp-oil-pump.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Oil pump SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '199px', objectFit: 'contain' }}
/>

The **HP Oil pump** widget represents an oil pump — a surface-mounted pumping unit used to lift crude oil from a well. The widget visualizes the pump across various operational states: it shows whether the pump is currently running and lifting fluid, and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on artificial lift management dashboards, well surveillance systems, or production optimization applications to monitor pump activity, surface equipment alarms, and pump-off conditions, and navigate to per-well detail views with a single click.

</div>

## Key capabilities

- Indicate active pumping operation in the running state.
- Indicate warning and critical states with configurable colors.
- Animate a blinking effect when the pump is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Oil pump widget has a pre-selected symbol from the **High-performance SCADA oil & gas** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the pump is currently stroking and lifting fluid. When `True`, the running state is shown. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the pump is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the pump is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the pump is in warning or critical state. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `pumpActive`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `pumpRunning` or `strokeActive`. |
...
The Appearance section controls the widget title, pump state colors, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Oil pump` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Running color** | Light gray | Pump color when actively stroking. |
| **Stopped color** | Dark gray | Pump color when idle or not lifting. |
...
<p style="font-size: 1.1em; font-weight: bold;">Pump shows no running state</p>
