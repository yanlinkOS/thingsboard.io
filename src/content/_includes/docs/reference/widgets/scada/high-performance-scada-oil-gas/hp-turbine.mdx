...
It represents an industrial gas turbine — a multi-stage axial rotating machine used for power generation or mechanical drive in oil and gas facilities. The widget shows whether the turbine is currently running at operating speed (running state), and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on power generation dashboards, pipeline compression station monitors, or offshore platform facility management systems to track turbine operating status, vibration and temperature alarms, and overspeed or surge events, and navigate to per-unit detail views with a single click.
