...
It represents a land-based drilling rig used in oil and gas exploration and production. The widget shows whether the rig is currently in active drilling operation (running state) and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on drilling operations dashboards, field management systems, or well lifecycle monitoring applications to track rig activity status, surface equipment fault or downtime alarms, and navigate to per-rig detail views with a single click.
