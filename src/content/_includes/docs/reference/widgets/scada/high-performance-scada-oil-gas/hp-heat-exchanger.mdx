...
The SCADA symbol defines the visual shape of the widget. The HP Heat exchanger widget has a pre-selected symbol from the **High-performance SCADA oil & gas** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the heat exchanger is currently in operation. When `True`, the running state is shown. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the heat exchanger is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the heat exchanger is in critical state. See [Data source configuration](#data-source-configuration). |
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getExchangerStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `exchangerActive`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `hxRunning` or `flowActive`. |
...
The Appearance section controls the widget title, exchanger state colors, and alarm colors.
...
<p style="font-size: 1.1em; font-weight: bold;">Heat exchanger shows no running state</p>
