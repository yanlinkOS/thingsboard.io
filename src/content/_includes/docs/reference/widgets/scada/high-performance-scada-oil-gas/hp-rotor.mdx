...
import symbolImg from '~/assets/images/reference/widgets/scada/high-performance-scada-oil-gas/symbol-hp-rotor.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="HP Rotor SCADA symbol"
  width={199}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '199px', height: '159px', objectFit: 'contain' }}
/>

The **HP Rotor** widget represents a rotary motor or top-drive unit used to rotate the drill string during drilling operations. The widget shows whether the rotor is currently spinning (running state) and highlights warning or critical conditions with configurable colors and a blinking animation. Use it on drilling operations dashboards or well monitoring systems to track rotor activity, surface mechanical fault or overload alarms, and navigate to detailed rotor telemetry with a single click.

</div>

## Key capabilities

- Indicate active rotor rotation in the running state.
- Indicate warning and critical states with configurable colors.
- Animate a blinking effect when the rotor is in warning or critical state.
...
The SCADA symbol defines the visual shape of the widget. The HP Rotor widget has a pre-selected symbol from the **High-performance SCADA oil & gas** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the rotor is currently spinning. When `True`, the running state is shown. See [Data source configuration](#data-source-configuration). |

**Warning state**

| Parameter | Default | Description |
|---|---|---|
| **Warning** | `Use alarm status` | Whether the rotor is in warning state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Critical** | `Use alarm status` | Whether the rotor is in critical state. See [Data source configuration](#data-source-configuration). |
...
| Parameter | Default | Description |
|---|---|---|
| **Warning/Critical state animation** | `False` | Whether to enable blinking animation when the rotor is in warning or critical state. See [Data source configuration](#data-source-configuration). |
...
- **Navigate to new dashboard state**
  ...
  Switches to a different view within the same dashboard, like opening a rotor details screen.
...
- **Update current dashboard state**
  ...
  Updates the context of the current view, like switching the selected rotor, without navigating away.
...
- **Navigate to other dashboard**
  ...
  Opens a completely different dashboard, like jumping from a rig overview to a rotating equipment monitoring dashboard.
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getRotorStatus`. |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `rotorActive`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `rotorRunning` or `driveState`. |
...
The Appearance section controls the widget title, rotor state colors, and alarm colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `HP Rotor` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Running color** | Light gray | Rotor color when actively spinning. |
| **Stopped color** | Dark gray | Rotor color when idle or stopped. |
| **Warning color** | Orange | Rotor color when in warning state. |
| **Critical color** | Red | Rotor color when in critical state. |
...
<p style="font-size: 1.1em; font-weight: bold;">Rotor shows no running state</p>
