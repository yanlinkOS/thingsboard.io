...
4. Select the drain pipe widget for your use case.
...
|---|---|---|
| **Leak** | `False` | Whether a leak is present on the drain pipe. See [Data source configuration](#data-source-configuration). |
...
- **Navigate to other dashboard**
  ...
  Opens a completely different dashboard, like jumping from a fluid system overview to a drain monitoring dashboard.
...
| Cause | Solution |
|---|---|
| **Leak** action is set to **Do nothing** and value is `False` | Open the **Leak** edit dialog, set the correct **Action**, and specify the attribute or time-series key. |
