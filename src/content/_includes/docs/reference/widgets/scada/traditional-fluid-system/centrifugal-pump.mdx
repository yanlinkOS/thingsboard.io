...
import symbolImg from '~/assets/images/reference/widgets/scada/traditional-fluid-system/symbol-centrifugal-pump.png';
import { Steps } from '@astrojs/starlight/components';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="Centrifugal pump SCADA symbol"
  width={160}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '160px', objectFit: 'contain' }}
/>
<div style={{ marginTop: 0 }}>

The **Centrifugal pump** widget represents a centrifugal pump with configurable pipe connectors, a running rotation animation, and color-coded warning and critical states.
...
- Animate the pump impeller rotation when the pump is in the running state.
...
- Configure which pipe connection points are visible on each side of the pump symbol.
...
The SCADA symbol defines the visual shape of the widget. The Centrifugal pump widget has a pre-selected symbol from the **Traditional SCADA fluid system** bundle. To use a different symbol, click **Browse SCADA symbol from gallery** and choose one from the library, or upload your own.
...
|---|---|---|
| **Running** | `Use attribute 'running'` | Whether the pump is currently running. When `True`, the impeller rotation animation plays. See [Data source configuration](#data-source-configuration). |
...
| **Rotation animation speed** | `1` | Speed of the impeller rotation animation. `1` is normal speed, `0` disables animation, `< 1` slower, `> 1` faster. See [Data source configuration](#data-source-configuration). |
...
Applies to **Running**, **Warning state**, **Critical state**, **Critical state animation**, and **Rotation animation speed**. Click the edit icon next to a parameter to open its configuration dialog. When **Action** is set to **Do nothing**, the **Value** options depend on which parameter you are configuring:
...
| **Rotation animation speed** | Any number — `1` for normal speed, `0` to stop, `< 1` - slower animation, `> 1` - faster animation. |
...
- **Execute RPC**
  ...
  | Parameter | Description |
  |---|---|
  | **Method** | RPC method to call, like `getState` or `getPumpStatus`. |
  ...
  | **'[parameter]' when result is** | The value or condition that activates this parameter (not shown for **Rotation animation speed**, which uses the raw numeric result). |
...
- **Get attribute**
  ...
  | **Attribute key** | Attribute name to read, like `running` or `pumpState`. |
...
- **Get time series**
  ...
  | Parameter | Description |
  |---|---|
  | **Time series key** | Telemetry key to monitor, like `pumpRunning` or `speed`. |
...
The Appearance section controls the widget title, visible pipe connectors, and pump state colors.

| Setting | Default | Description |
|---|---|---|
| **Title** | `Centrifugal pump` | Widget title displayed in the header. Supports `entityName` and `entityLabel` variables. |
| **Card icon** | — | Icon shown in the widget header. Configure its size and color. |
| **Right top connector** | Enabled | Show or hide the pipe connection point on the top-right side of the pump. |
| **Right bottom connector** | Disabled | Show or hide the pipe connection point on the bottom-right side of the pump. |
| **Left top connector** | Disabled | Show or hide the pipe connection point on the top-left side of the pump. |
| **Left bottom connector** | Enabled | Show or hide the pipe connection point on the bottom-left side of the pump. |
| **Top left connector** | Disabled | Show or hide the pipe connection point on the top-left corner of the pump. |
| **Top right connector** | Disabled | Show or hide the pipe connection point on the top-right corner of the pump. |
...
<p style="font-size: 1.1em; font-weight: bold;">Pump shows no rotation animation when running</p>
