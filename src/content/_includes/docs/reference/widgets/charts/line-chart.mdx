...
import symbolImg from '~/assets/images/reference/widgets/charts/line-chart.svg';

<div style={{ overflow: 'auto', marginBottom: '1.5rem' }}>
<Image
  src={symbolImg}
  alt="Line chart widget"
  width={200}
  style={{ float: 'left', marginRight: '1.25rem', marginBottom: '0.5rem', width: '200px', objectFit: 'contain' }}
/>
<div style={{ marginTop: 0 }}>

The **Line chart** widget visualizes time-series telemetry as lines. It supports real-time updates, multiple series, dual Y axes, data aggregation, thresholds, and comparison mode — making it ideal for tracking metrics such as temperature, pressure, voltage, or energy consumption over time.
...
- Multiple data series with independent colors, line styles, and Y axis assignments.
...
Each data key is rendered as an independent line. Add a key for each telemetry value to display, like `temperature` or `humidity`. Each line updates dynamically as new telemetry arrives.
...
The Line chart supports the <DocLink product={props.product} path="user-guide/actions/#widget-header-button" bold={false} target="_blank">Widget header button</DocLink> action — a button placed in the widget header that triggers a configured action when clicked.
...
<MultiProductImageGallery product={props.product} images={[
  { src: '/src/assets/images/reference/widgets/charts/line-chart/temperature-and-humidity-chart.png', alt: 'Line chart showing temperature and humidity over 12 hours', caption: 'Temperature and humidity telemetry from a single device with separate Y axes.', products: [Products.CE] },
]} />
...
**Prerequisites:** A device named **Thermo-hygrometer** that sends `temperature` (°C) and `humidity` (%) as telemetry keys.
...
**Step 1. Add the widget.** Open your dashboard in edit mode, click **Add widget**, select the **Charts** bundle, then choose **Line chart**.

**Step 2. Configure the time window.** Set **Period** to Realtime — last **12 hours**, **Aggregation** to Average, and **Grouping interval** to 30 minutes.

**Step 3. Configure the datasource.** Set **Datasource type** to **Device**, then select **Thermo-hygrometer**.
...
For each key, open advanced settings, enable **Smooth line**, and set **Line width** to **3**.
...
**Step 6. Configure appearance and legend.** In the **Appearance** section set **Title** to `Temperature and Humidity`, font size **18 px**, weight **600**. In the **Legend** section set **Position** to **Top** and enable **Min**, **Max**, **Average**.

**Step 7. Save.** Click **Add**, resize and reposition as needed, then click **Save** on the dashboard toolbar.

**Result:** The chart displays two lines updated in real time — an orange line for temperature and a blue line for humidity — tracked over 12 hours. The legend shows min/max/avg statistics for each series; the tooltip displays exact readings at any hovered timestamp.
