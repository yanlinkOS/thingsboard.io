...
The **Speed gauge** is a semicircular dial styled after a classic speedometer, with color-coded zones that escalate from neutral through yellow to orange as values approach the configured maximum. Use it to convey how close a measured rate is to its operational limit — conveyor belt speed, motor RPM, wind speed, or network throughput where operators must immediately recognize when a value enters a warning or critical zone.
