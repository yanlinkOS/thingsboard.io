...
Other HTTP features can be implemented manually with `sendGetRequest` or `sendPostRequest`. See the <DocLink product={props.product} path='reference/http-api/getting-connected'>HTTP API</DocLink> reference.
...
See the `examples` folder in the [thingsboard-client-sdk](https://github.com/thingsboard/thingsboard-client-sdk/tree/master/examples) repository, including the [ESP32 Pico Kit GPIO control and DHT22 sensor](/docs/samples/esp32/gpio-control-pico-kit-dht22-sensor/) tutorial.
