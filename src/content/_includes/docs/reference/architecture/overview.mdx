...
<DocLink product={props.product} path="user-guide/multi-tenancy/#tenant-profiles">Tenant profiles</DocLink> define per-tenant resource limits and quotas:
...
```txt
 ...
 └────────┘                   └─────┬─────┘                  └──────┬──────┘
                              ...
                              (credentials                   Filter, enrich,
                               from cache)                   transform, act
                                                                    │
                                    ┌───────────────────────────────┘
                                    │
                                    ▼
...
```txt
 ...
 │ User /   │──────────────▶│  TB Node     │──── Queue ─────▶│ Transport │───  specific ──▶│ Device │
 │ Rule     │   or Rule     │  (Core +     │                  │           │     command     └────────┘
 │ Chain    │   Chain action│   Actors)    │                  └───────────┘
 └──────────┘               └──────────────┘
```
...
```txt
 ┌───────────┐     WS      ┌──────────────┐     Subscription     ┌──────────────┐
 │ Dashboard │◀────────────▶│  TB Node     │◀────────────────────▶│  TB Node     │
 │ (Browser) │   subscribe  │  (handles WS │    notifications     │  (processes  │
 │           │   + updates   │   session)   │    via Kafka         │   telemetry) │
 └───────────┘              └──────────────┘                      └──────────────┘
```
