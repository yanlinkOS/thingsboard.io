...
- **From**: `no-reply@yourcompany.com` — static sender address
...
- **Metadata**: `{ "userEmail": "johndoe@thingsboard.io", "isHtml": "true" }`

**Outgoing `SEND_EMAIL` data**:

```json maxLines=12 collapsible
{
  "from": "no-reply@yourcompany.com",
  "to": "johndoe@thingsboard.io",
  "cc": null,
  "bcc": null,
...
`${userEmail}` resolved to `johndoe@thingsboard.io`; `${isHtml}` resolved to `"true"` → `html: true` in output.
