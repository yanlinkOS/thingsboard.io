...
Each user belongs to a specific <DocLink product={props.product} path='user-guide/multi-tenancy/' target="_blank">Tenant</DocLink> or a <DocLink product={props.product} path='user-guide/customers/' target="_blank">Customer</DocLink> owner{props.product === Products.PE && " and are assigned to one or more user groups that determine their permissions and roles"}.  
Ownership determines the scope of data and resources available to the user.

#### Access control model

<br></br>{props.product === Products.PE && <>ThingsBoard Professional  Edition uses a <DocLink product={props.product} path='user-guide/roles/' target="_blank">Role-Based Access Control (RBAC)</DocLink> model:
  <ul>
