...
|---|---|---|
| `type` | `String` | Subscription type: `'timeseries'`, `'latest'`, or `'alarm'` |
...
**`type`** determines what data is returned:
...
- **`subscriptionOptions.type: 'latest'`** — returns only the most recent value per key. Use `'timeseries'` instead if you need the full history over a time window.
...
- **`type: 'timeseries'`** — delivers the full history of `dataKeys` over the configured time window. Each entry in `ctx.data[i].data` is a `[timestamp, value]` pair, sorted oldest to newest.
...
- **`type: 'alarm'`** — alarm subscriptions use `alarmSource` instead of `datasources`. The `dataKeys` inside `alarmSource` use `type: 'alarm'` and reference alarm fields by name (`createdTime`, `originator`, `type`, `severity`, `status`).
